...
const NPC = function(isFemale) {
  ...
  res.heading = function(dir) { return NPC_HEADING(this, dir); };

  // This does not work properly, it just gets all clothing!!!
  // But authors could replace as required
  res.getWearingVisible = function() { return this.getWearing(); };

  res.getTopics = npc_utilities.getTopics;

  res.isHere = function() { return this.isAtLoc(game.player.loc); }
    ...
    if (this.agenda)
      this.customSaveAgenda = this.agenda.join("^");
    this.preSave();
  };

  res.templatePostLoad = function() {
    if (this.customSaveAgenda)
      this.agenda = this.customSaveAgenda.split("^");
    delete this.customSaveAgenda;
    if (this.leaderName)
      w[this.leaderName].followers.push(this);
    this.postLoad();
  };
  ...
  res.doEvent = function() {
    if (this.isHere()) {
      if (typeof this.reactions === "function") {
        ...
        for (let key in this.reactions) {
          console.log("key:" + key);
          if (this.reactionFlags.split(" ").includes(key))
            continue;
    ...
    if (this.followers.length !== 0) {
      ...
      this.savedAlias = this.alias this.pronouns = PRONOUNS.plural;
    ...
    if (this.savedPronouns) {
      ...
      this.alias = this.savedAlias delete this.savedPronouns;
    }
  };
  ...
  res.topics =
    ...
    if (arr.length !== 0) {
      metamsg("Some suggestions for what to ask " +
              this.byname({article : DEFINITE}) +
              " about: " + arr.sort().join("; ") + ".");
      flag = true;
    }
    ...
    if (arr.length === 0 && !flag) {
      metamsg("No suggestions for what to ask or tell " +
              this.byname({article : DEFINITE}) + " available.");
    } else if (arr.length !== 0) {
      metamsg("Some suggestions for what to tell " +
              this.byname({article : DEFINITE}) + " about: " +
              arr.sort().join("; ") + " (other topics may also be available).");
    }
...
const npc_utilities = {
  ...
  getTopics : function() {
    const list = [];
    for (let key in w) {
      if (w[key].isTopicVisible && w[key].isTopicVisible(this.name)) {
        list.push(w[key]);
      }
    }
    return list;
...
const agenda = {
  ...
  setItemAtt : function(npc, arr) {
    ...
    if (!w[item])
      errormsg("Item '" + item + "' not recognised in the agenda of " +
               npc.name);
    if (value === "true")
      value = true;
    if (value === "false")
  ...
  wait : function(npc, arr) {
    if (arr.length === 0)
      return true;
    if (isNaN(arr[0]))
      errormsg("Expected wait to be given a number in the agenda of '" +
               npc.name + "'");
  ...
  // Move the given item directly to the given location, then print the rest of
  // the array as text
  ...
  moveItem : function(npc, arr) {
    ...
    // debugmsg("dest:" + dest);
    if (!w[item])
      errormsg("Item '" + item + "' was not recognised in the agenda of " +
               npc.name);
    if (!w[dest])
      errormsg("Location '" + dest + "' was not recognised in the agenda of " +
               npc.name);
  ...
  // Move directly to the given location, then print the rest of the array as
  // text
  ...
  moveTo : function(npc, arr) {
    ...
    if (!w[dest])
      debugmsg("Location '" + dest + "' not recognised in the agenda of " +
               npc.name);
    if (!w[dest].room)
      dest = dest.loc;
    if (!w[dest])
      errormsg("Location '" + dest + "' not recognized in the agenda of " +
               npc.name);
  ...
  // Move to another room via a random, unlocked exit, then print the rest of
  // the array as text
  walkRandom : function(npc, arr) {
    ...
    if (!w[exit.name])
      errormsg("Location '" + exit.name + "' not recognised in the agenda of " +
               npc.name);
    npc.moveWithDescription(exit.name);
    return false;
  },

  // Move to the given location, using available, unlocked exits, one room per
  // turn
  // then print the rest of the array as text
  // Use "player" to go to the room the player is in (if the player moves, the
  // NPC will head
  ...
  walkTo : function(npc, arr) {
    ...
    if (dest === "player")
      dest = game.player.loc;
    if (w[dest] === undefined) {
      errormsg("Location '" + dest + "' not recognised in the agenda of " +
               npc.name);
      return true;
    }
    if (!w[dest].room) {
      dest = w[dest].loc;
      if (w[dest] === undefined) {
        errormsg("Object location '" + dest +
                 "' not recognised in the agenda of " + npc.name);
        return true;
      }
    }
    if (npc.isAtLoc(dest)) {
      ...
      if (!route)
        errormsg("Location '" + dest + "' not reachable in the agenda of " +
                 npc.name);
  ...
  while (currentList.length > 0 && length < maxlength) {
    ...
    for (let i = 0; i < currentList.length; i++) {
      ...
      for (let j = 0; j < exits.length; j++) {
        ...
        dest = w[exits[j].name];
        if (dest === undefined)
          errormsg("Dest is undefined: " + exits[j].name);
  ...
  const res = {
    ...
    runscript : function() {
      ...
      if (typeof this.nowShow === "string")
        errormsg("nowShow for topic " + this.nname + " is a string.");
      for (let i = 0; i < this.nowShow.length; i++) {
        obj = w[this.nowShow[i]];
        if (obj === undefined)
          errormsg("No topic called " + this.nowShow[i] + " found.");
        obj.showTopic = true;
      }
      if (typeof this.nowHide === "string")
        errormsg("nowHide for topic " + this.nname + " is a string.");
      for (let i = 0; i < this.nowHide.length; i++) {
        obj = w[this.nowHide[i]];
        if (obj === undefined)
          errormsg("No topic called " + this.nowHide[i] + " found.");
        obj.hideTopic = true;
      }
      this.count++;
    ...
    isTopicVisible : function(char) {
      return this.showTopic && !this.hideTopic && char === this.loc;
    },
    show : function() { return this.showTopic = true; },
    hide : function() { return this.hideTopic = true; },
  };
  return res;
};