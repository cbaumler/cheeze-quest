...
// This is not language neutral, but should not be shipping with the game, so
// tough
...
test.assertCmd = function(cmdStr, expected, extraOutput) {
  ...
  if (test.testOutput.length === expected.length &&
      test.testOutput.every(function(value, index) {
    ...
    test.printTitle();
    for (let i = 0; i < Math.max(test.testOutput.length, expected.length);
         i++) {
      if (typeof expected[i] === "string") {
        ...
        if (test.testOutput[i] === undefined ||
...
test.results = function(time) {
  ...
  debugmsg("Elapsed time: " + elapsed + " ms (" +
           (Math.round(elapsed / test.totalCount * 10) / 10) + " ms/test)");
};
