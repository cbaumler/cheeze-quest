...
function msg(s, params, cssClass) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (cssClass === undefined) {
      ...
      io.outputRaw('<p id="n' + io.nextid + '" class="' + cssClass + '">' +
                   lines[i] + "</p>");
    }
    io.nextid++;
    if (game.spoken)
      io.speak(lines[i]);
    if (game.transcript)
      game.scriptAppend("-" + lines[i]);
  }
}

// Just the same as msg, but adds the "failed" CSS class. This allows failed
// command responses to be differentiated.
function failedmsg(s, params) { msg(s, params, "failed"); }

// Output a meta-message - a message to inform the player about something
// outside the game world, such as hints and help messages. The string will
// first be passed through the text processor. Additional data can be put in the
// optional params dictionary. During unit testing, messages will be saved and
// tested
function metamsg(s, params) {
  ...
  if (game.spoken)
    io.speak(processed);
  if (game.transcript)
    game.scriptAppend("M" + processed);
}

// Output an error message.
// Use for when something has gone wrong, but not when the player types
// something odd - if you see this during play, there is a bug in your game (or
// my code!) During unit testing, error messages will be saved and tested. Does
// not use the text processor.
function errormsg(s) {
  if (test.testing) {
  ...
  if (game.spoken)
    io.speak("Error: " + s);
  if (game.transcript)
    game.scriptAppend("E" + s);
}

// Output a message from the parser indicating the input text could not be
// parsed. During unit testing, messages will be saved and tested. Does not use
// the text processor.
function parsermsg(s) {
  ...
  io.nextid++;
  if (game.spoken)
    io.speak("Parser error: " + s);
  if (game.transcript)
    game.scriptAppend("P" + s);
}
...
function heading(level, s, params) {
  const processed = processText(s, params);
  if (!test.testing) {
    io.outputRaw('<h' + level + ' id="n' + io.nextid + '">' + processed +
                 '</h' + level + '>');
    io.nextid++;
  }
  if (game.spoken)
    io.speak(s);
  if (game.transcript)
    game.scriptAppend(level.toString() + s);
}
...
function printOrRun(char, item, attname, options) {
  ...
  if (Array.isArray(attname)) {
    ...
    if (typeof attname === "function") {
      return attname(item, options.multi, char, options.match);
    } else {
    ...
    msg(prefix(item, options.multi) + item[attname],
        {char : char, item : item});
    return true;
  } else if (typeof item[attname] === "function") {
    // The attribute is a function
    const res = item[attname](options.multi, char, options.match);
    return res;
  } else {
...
// Use like this:
//      showMenu('What is your favourite color?', ['Blue', 'Red', 'Yellow',
//      'Pink'], function(result) {
...
function showMenu(title, options, fn) {
  ...
  if (test.testing) {
    if (test.menuResponseNumber === undefined ||
        test.menuResponseNumber >= options.length) {
      debugmsg(
          "Error when testing menu (possibly due to disambiguation?), test.menuResponseNumber = " +
          test.menuResponseNumber);
    } else {
...
function showDropDown(title, options, fn) {
  ...
  s +=
      'onchange=\"io.menuResponse($(\'#menu-select\').find(\':selected\').val())\">';
...
// This should be called after each turn to ensure we are at the end of the page
// and the text box has the focus
function endTurnUI(update) {
  // debugmsg("In endTurnUI");
  if (PANES !== 'None' && update) {
    // set the EXITS
    for (let i = 0; i < EXITS.length; i++) {
      if (game.room.hasExit(EXITS[i].name, {excludeScenery : true}) ||
          EXITS[i].nocmd) {
    ...
    io.updateStatus();
    io.updateUIItems();
    if (typeof ioUpdateCustom === "function")
      ioUpdateCustom();
  }
...
function cmdLink(command, str) {
  return '<a class="cmdlink" onclick="parser.parse(\'' + command + '\')">' +
         str + "</a>";
}
...
io.outputRaw = function(s) { $("#output").append(s); }
...
io.rect = function(x, y, w, h, c) {
  return '<rect width="' + w + '" height="' + h + '" x="' + x + '" y="' + y +
         '" style="fill:' + c + ';stroke-width:1;stroke:black" />';
};
...
io.setTitle = function(s) { document.title = s; };
...
io.updateUIItems = function() {
  ...
  for (let key in w) {
    ...
    for (let i = 0; i < INVENTORIES.length; i++) {
      ...
      if (INVENTORIES[i].test(item) && !item.inventorySkip) {
  ...
  for (let i = 0; i < STATUS.length; i++) {
    if (typeof STATUS[i] === "string") {
      if (game.player[STATUS[i]] !== undefined) {
        let s = '<tr><td width="' + STATUS_WIDTH_LEFT + '">' +
                sentenceCase(STATUS[i]) + "</td>";
        s += '<td width="' + STATUS_WIDTH_RIGHT + '">' +
             game.player[STATUS[i]] + "</td></tr>";
...
io.clickToContinueLink =
    function() {
  msg('<a class="continue" onclick="io.waitContinue()">' + CLICK_TO_CONTINUE +
      '</a>');
...
io.clickItemAction = function(itemName, action) {
  ...
  if (cmd === undefined) {
    errormsg(
        "I don't know that command (" + action +
        ") - and obviously I should as you just clicked it. Please alert the game author about this bug.");
    console.log("Click action failed")
    console.log("Action: " + action)
    console.log("Item: " + itemName)
  } else if (item === undefined) {
    errormsg(
        "I don't know that object (" + itemName +
        ")- and obviously I should as it was listed. Please alert this as a bug in Quest.");
  } else {
...
io.appendItem = function(item, htmlDiv, loc, isSubItem) {
  ...
  $('#' + htmlDiv)
      .append('<p class="item' + (isSubItem ? ' subitem' : '') +
              '" onclick="io.clickItem(\'' + item.name + '\')">' + item.icon() +
              item.getListAlias(loc) + "</p>");
  io.currentItemList.push(item.name);
  const verbs = item.getVerbs(loc);
  for (let j = 0; j < verbs.length; j++) {
    let s = '<div class="' + item.name +
            '-actions itemaction" onclick="io.clickItemAction(\'' + item.name +
            '\', \'' + verbs[j] + '\')">';
    s += verbs[j];
    s += '</div>';
    $('#' + htmlDiv).append(s);
  ...
  if (item.container && !item.closed) {
    ...
    for (let i = 0; i < l.length; i++) {
      io.appendItem(l[i], htmlDiv, item.name, true);
    }
  }
};
...
io.createPanes = function() {
  ...
  if (COMPASS) {
    if (typeof DIVIDER !== 'undefined')
      document.writeln('<img src="images/' + DIVIDER + '" />');
  ...
  if (typeof ioCreateCustom === "function")
    ioCreateCustom();
};

io.writeExit = function(n) {
  document.writeln('<td class="compassbutton">');
  document.writeln('<span class="compassbutton" id="exit' + EXITS[n].name +
                   '" onclick="io.clickExit(\'' + EXITS[n].name + '\')">' +
                   EXITS[n].abbrev + '</span>');
  document.writeln('</td>');
};
...
io.getCommand = function(name) {
  const found = commands.find(function(el) { return el.name === name; });
  return found;
};
...
io.msgInputText = function(s) {
  if (!CMD_ECHO)
    return;
  $("#output").append('<p id="n' + io.nextid + '" class="inputtext">&gt; ' + s +
                      "</p>");
  io.nextid++;
  if (game.spoken)
    io.speak(s, true);
  if (game.transcript)
    game.scriptAppend("I" + s);
};
  ...
  $('#textbox').keydown(function(event) {
    ...
    if (keycode === 38) {
      ...
      if (event.ctrlKey) {
        ...
        const el = $('#textbox')[0] if (el.setSelectionRange) {
...
io.speak = function(str, altVoice) {
  ...
  if (!io.voice) {
    io.voice = io.synth.getVoices().find(function(
        el) { return /UK/.test(el.name) && /Female/.test(el.name); });
    if (!io.voice)
      io.voice = io.synth.getVoices()[0];
  }
  if (!io.voice2) {
    io.voice2 = io.synth.getVoices().find(function(
        el) { return /UK/.test(el.name) && /Male/.test(el.name); });
    if (!io.voice2)
...
io.showHtml = function(title, html) {
  if (io.dialogShowing)
    return false;
  $('body').append('<div id="showHtml" title="' + title + '">' + html +
                   '</div>');
