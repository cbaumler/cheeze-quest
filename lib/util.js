...
function randomChance(percentile) { return randomInt(99) < percentile; }

// Returns a random element from the array, or null if it is empty
// If the second parameter is true, then the selected value is also deleted from
// the array, preventing it from being selected a second time
...
function replaceAll(str, regex, replace) { return str.replace(regex, replace); }
...
function formatList(itemArray, options) {
  ...
  if (!options.lastJoiner) {
    options.lastJoiner = options.sep;
  }
  ...
  if (!options.separateEnsembles) {
    ...
    for (let i = 0; i < itemArray.length; i++) {
      if (itemArray[i].ensembleMaster &&
          itemArray[i].ensembleMaster.isAllTogether()) {
        toRemove.push(itemArray[i]);
        if (!toAdd.includes(itemArray[i].ensembleMaster))
          toAdd.push(itemArray[i].ensembleMaster);
      }
    }
...
function listProperties(obj) { return Object.keys(obj).join(", "); }
...
function displayMoney(n) {
  ...
  if (ary.length === 2) {
    ...
    const negative = (n < 0)
    n = Math.abs(n);
    let options = ary[1] const showsign = options.startsWith("+")
    if (showsign) {
    ...
    errormsg(
        "MONEY_FORMAT in setting.js expected to have either 1, 2 or 3 exclamation marks.")
    return "" + n;
  }
}
...
function displayNumber(n, control) {
  ...
  if (!regex.test(control)) {
    errormsg(
        "Unexpected format in displayNumber (" + control +
        "). Should be a number, followed by a single character separator, followed by a number.");
    return "" + n;
  }
  ...
  if (places > 0) {
    // We want a decimal point, so the padding, the total length, needs that
    // plus the places
  ...
  return (options[1] + decimal.padStart(padding, "0") +
          options[5]) // eg "(012,34)"
}

// Should convert the string (what the player typed) to the actual direction,
...
function getDir(s) {
  for (let i = 0; i < EXITS.length; i++) {
    ...
    if (EXITS[i].name === s)
      return EXITS[i].name;
    if (EXITS[i].abbrev.toLowerCase() === s)
      return EXITS[i].name;
    if (new RegExp("^(" + EXITS[i].alt + ")$").test(s))
      return EXITS[i].name;
  }
  return false;
}
...
function scopeHereListed() {
  const list = [];
  for (let key in w) {
    if (w[key].isAtLoc(game.player.loc, display.LOOK) &&
        world.ifNotDark(w[key])) {
      list.push(w[key]);
    }
  }
  return list;
...
const util = {}
             ...
             util.isHeldNotWorn =
    function(item) {
  return item.isAtLoc(game.player.name, display.SIDE_PANE) &&
         world.ifNotDark(item) && !item.getWorn();
} util.isHere =
        function(item) {
  return item.isAtLoc(game.player.loc, display.SIDE_PANE) &&
...
const getContents = function(situation) {
  const list = [];
  for (let key in w) {
...
function arraySubtract(a, b) {
  const res = [] for (let i = 0; i < a.length; i++) {
    if (!b.includes(a[i]))
      res.push(a[i]);
  }
  return res;
