...
function createObject(name, listOfHashes) {
  if (world.isCreated && !SAVE_DISABLED) {
    console.log(
        "Attempting to use createObject with `" + name +
        "` after set up. To ensure games save properly you should use cloneObject to create ites during play.")
    errormsg(
        "Attempting to use createObject with `" + name +
        "` after set up. To ensure games save properly you should use cloneObject to create ites during play.");
    return null;
  ...
  if (/\W/.test(name)) {
    console.log(
        "Attempting to use the disallowed name `" + name +
        "`; a name can only include letters and digits - no spaces or accented characters. Use the 'alias' attribute to give an item a name with other characters.")
    errormsg(
        "Attempting to use the disallowed name `" + name +
        "`; a name can only include letters and digits - no spaces or accented characters. Use the 'alias' attribute to give an item a name with other characters.");
    return null;
  ...
  if (w[name]) {
    console.log("Attempting to use the name `" + name +
                "` when there is already an item with that name in the world.")
    errormsg("Attempting to use the name `" + name +
             "` when there is already an item with that name in the world.");
    return null;
  }
...
const world = {
  ...
  init : function() {
    ...
    // This handles the player wanting to interact with things in room
    // descriptions that are not implemented by changing its regex when a room
    // is entered.
    if (w.background === undefined) {
      w.background = createItem("background", {
        ...
        isAtLoc : function(
            loc,
            situation) { return w[loc].room && situation === display.PARSER; },
      });
    }
  ...
  initItem : function(item) {
    if (item.loc && !w[item.loc]) {
      errormsg("The item `" + item.name + "` is in an unknown location (" +
               item.loc + ")");
    }
  ...
  // Call after the player takes a turn, sending it a result, SUCCESS,
  // SUCCESS_NO_TURNSCRIPTS or FAILED Turnscripts are just objects in the
  // world.data array with a "turnscript" attribute set to a function and a
  // Boolean "runTurnscript".
  ...
  ifNotDark : function(
      item) { return (!game.dark || item.lightSource() > LIGHT_NONE); },
  ...
  enterRoom : function() {
    if (game.room.beforeEnter === undefined) {
      errormsg(
          "This room, " + game.room.name +
          ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.");
      return;
    }
    game.room.beforeEnter();
    if (game.room.visited === 0) {
      game.room.beforeFirstEnter();
    }
  ...
  // Called when entering a new room, after beforeEnter and beforeFirstEnter re
  // done
  enterRoomAfterScripts : function() {
    if (ROOM_HEADINGS)
      heading(4, sentenceCase(game.room.alias));
    if ((game.verbosity === TERSE && game.room.visited === 0) ||
        game.verbosity === VERBOSE) {
      game.room.description();
    }
    game.room.afterEnter();
    if (game.room.visited === 0) {
      game.room.afterFirstEnter();
    }
    if (game.room.afterEnterIf) {
      for (let key in game.room.afterEnterIf) {
        if (game.room.afterEnterIfFlags.split(" ").includes(key))
          continue;
  ...
  setBackground : function() {
    ...
    if (typeof game.room.desc === 'string') {
      if (!game.room.backgroundNames) {
        ...
        while (md = world.BACK_REGEX.exec(
                   game.room.desc)) { // yes it is an assignment!
    ...
    w.background.regex =
        (game.room.backgroundNames && game.room.backgroundNames.length > 0)
            ? new RegExp(game.room.backgroundNames.join("|"))
            : false;
  },
...
const game = createObject("game", [
  {
    ...
    update : function(player) {
      ...
      if (!this.player.loc || !w[this.player.loc]) {
        errormsg(
            "No player location set or set to location that does no exist. This will not go well...");
      }
    ...
    saveGameState : function() {
      if (MAX_UNDO > 0) {
        ...
        if (this.gameState.length > MAX_UNDO)
    ...
    scriptShow : function(opts) {
      ...
      for (let i = 0; i < this.transcriptText.length; i++) {
        ...
        case 'M':
          if (!opts.includes('m')) {
            html += '<p style="color:blue">' + s + '</p>';
          }
          break;
        case 'E':
          if (!opts.includes('e')) {
            html += '<p style="color:red">' + s + '</p>';
          }
          break;
        case 'D':
          if (!opts.includes('d')) {
            html += '<p style="color:grey">' + s + '</p>';
          }
          break;
        case 'P':
          if (!opts.includes('p')) {
            html += '<p style="color:magenta">' + s + '</p>';
          }
          break;
        case 'I':
          if (!opts.includes('i')) {
            html += '<p style="color:cyan">' + s + '</p>';
          }
          break;
        default:
          html += '<h' + this.transcriptText[i][0] + '>' + s + '</h' +
                  this.transcriptText[i][0] + '>';
          break;
        }
      }
    ...
    scriptAppend : function(s) { this.transcriptText.push(s); },

    timerEvents : [], // These will not get saved!!!
    eventFunctions : {},
    registerEvent : function(eventName,
                             func) { this.eventFunctions[eventName] = func; },
    registerTimedEvent : function(eventName, triggerTime, interval) {
      this.timerEvents.push({
        eventName : eventName,
        triggerTime : triggerTime + game.elapsedRealTime,
        interval : interval
      });
    },
    gameTimer : function() {
      // Note that this gets added to window by setInterval, so 'this' does not
      // refer to the game object
      game.elapsedRealTime++;
      for (let i = 0; i < game.timerEvents.length; i++) {
        if (game.timerEvents[i].triggerTime &&
            game.timerEvents[i].triggerTime < game.elapsedRealTime) {
          if (typeof (game.eventFunctions[game.timerEvents[i].eventName]) ===
              'function') {
            ...
            errormsg("A timer is trying to call event '" +
                     game.timerEvents[i].eventName +
                     "' but no such function is registered.");
    ...
    preSave : function() {
      ...
      for (let i = 0; i < game.timerEvents.length; i++) {
        if (game.timerEvents[i].triggerTime) {
          arr.push(game.timerEvents[i].eventName + "/" +
                   game.timerEvents[i].triggerTime + "/" +
                   (game.timerEvents[i].interval ? game.timerEvents[i].interval
                                                 : '-'));
        }
      }
