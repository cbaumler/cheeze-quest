...
// Lists of multiple objects must be separated by JOINER_REGEX (i.e., "and" or
// comma) Item names cannot include the word "and" or commas Commands look in
// their specified scope first, then isVisible as the fallback. It will try to
// match the beginning of a word to the object name given.
//    If the player does GET H, then it will disambiguate between any objects
//    beginning with H
// A match is scored by whether the objects are in the right place (+2), wrong
// place but here (+1). You can also give a command a score attribute to boost
// (or reduce) its success.

// Should all be language neutral (except the inspect function, which is just
// for debugging)
...
// If sent null it will continue to work with the current values in
// currentCommand. This allows us to keep trying to process a single command
// until all the
...
parser.parse = function(inputText) {
  ...
  // Need to disambiguate, until each of the lowest level lists has exactly one
  // member
  let flag = false;
    ...
    for (let j = 0; j < parser.currentCommand.objects[i].length; j++) {
      if (parser.currentCommand.objects[i][j] instanceof Array) {
        if (parser.currentCommand.objects[i][j].length === 1) {
          parser.currentCommand.objects[i][j] =
              parser.currentCommand.objects[i][j][0];
        } else {
          flag = true;
          parser.currentCommand.disambiguate1 = i;
          parser.currentCommand.disambiguate2 = j;
          showMenu(DISAMBIG_MSG, parser.currentCommand.objects[i][j],
                   function(result) {
                     parser.currentCommand
                         .objects[parser.currentCommand
                                      .disambiguate1][parser.currentCommand
                                                          .disambiguate2] =
                         result;
                     parser.parse(null);
                   });
...
// You can use this to bypass all the text matching when you know what the
// object and command are Used by the panes when the player clicks on a verb for
// an item
...
parser.execute = function() {
  ...
  if (parser.currentCommand.objects.length > 0 &&
      typeof parser.currentCommand.objects[0] === "object") {
    for (let i = 0; i < parser.currentCommand.objects[0].length; i++) {
      parser.pronouns[parser.currentCommand.objects[0][i].pronouns.objective] =
          parser.currentCommand.objects[0][i];
    }
  }
  // console.log(parser.currentCommand);
  const outcome = parser.currentCommand.cmd.script(
      parser.currentCommand.objects, parser.currentCommand.matches);
...
// .objects        a list (of a list of a list), one member per capture group in
// the command regex .objects[0]     a list (of a list), one member per object
// name given by the player for capture group 0 .objects[0][0]  a list of
// possible object matches for each object name given by the player for the
//                      first object name in capture group 0
parser.convertInputTextToCommandCandidate = function(inputText) {
  // let s = inputText.toLowerCase().split(' ').filter(function(el) { return
  // !IGNORED_WORDS.includes(el); }).join(' ');
  ...
  const candidates =
      commands.filter(function(el) { return el.regex.test(cmdString); });
  ...
  candidates.forEach(function(el) {
    // matchItemsToCmd will attempt to fit the objects, returns a dictionary if
    // successful or an error message otherwise. Could have more than one
    // object, either because multiple were specified or because it was
    // ambiguous (or both) We just keep the last error message as hopefully the
    // most relevant. NB: Inside function so cannot use 'this'
    if (PARSER_DEBUG)
      console.log(el);
...
// of possible object matches (so GET HAT is four if there are four hats in the
// room)
parser.matchItemsToCmd = function(s, cmd) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (cmd.objects[i].text) {
      ...
      let list = cmd.objects[i].scope ? parser.scope(cmd.objects[i].scope)
                                      : fallbackScope;
      ...
      if (ALL_EXCLUDE_REGEX.test(arr[i])) {
        ...
        // if it is ambiguous or not recognised it does not get added to the
        // list
        let s = arr[i].replace(ALL_EXCLUDE_REGEX, "").trim();
        objectNames =
            s.split(JOINER_REGEX).map(function(el) { return el.trim(); });
        for (let j = 0; j < objectNames.length; j++) {
          let items = parser.findInList(objectNames[j], fallbackScope);
          if (items.length === 1) {
      ...
      objectNames =
          arr[i].split(JOINER_REGEX).map(function(el) { return el.trim() });
      if (objectNames.length > 1 && !cmd.objects[i].multiple) {
        res.error = NO_MULTIPLES_MSG;
        res.score = -1;
        return res;
      }
      let scopes = cmd.objects[i].scope
                       ? [ parser.scope(cmd.objects[i].scope), fallbackScope ]
                       : [ fallbackScope ];
      ...
      for (let j = 0; j < objectNames.length; j++) {
        ...
        if (objNameMatch === null) {
          errormsg("Failed to match to ARTICLE_FILTER_REGEX with '" +
                   objectNames[j] +
                   "', - probably an error in ARTICLE_FILTER_REGEX!");
          return null;
        }
        [objs2, n] = this.findInScope(objNameMatch[1], scopes, cmd.objects[i]);
        if (n === 0) {
          res.error = (cmd.noobjecterror ? cmd.noobjecterror
                                         : OBJECT_UNKNOWN_MSG(objectNames[j]));
          res.score = -1;
          return res;
        } else {
          if (n > score) {
...
// It will return a list of matching objects (to be disambiguated if more than
// 1), plus the score, depending on which list the object(s) was found in (if
// there are three lists, the score will be 3 if found in the first list, 2 in
// the second, or 1 if in the third list). If not found the score will be 0, and
// an empty array returned.
parser.findInScope = function(s, listOfLists, cmdParams) {
  // First handle IT etc.
  for (let key in PRONOUNS) {
    if (s === PRONOUNS[key].objective &&
        parser.pronouns[PRONOUNS[key].objective]) {
...
parser.inspect = function() {
  ...
  s += "Objects/texts (" + parser.currentCommand.objects.length + "):" +
       "<br/>";
  for (let i = 0; i < parser.currentCommand.objects.length; i++) {
    if (typeof parser.currentCommand.objects[i] === "string") {
      s += "&nbsp;&nbsp;&nbsp;&nbsp;Text: " + parser.currentCommand.objects[i] +
           "<br/>";
    } else {
      s += "&nbsp;&nbsp;&nbsp;&nbsp;Objects:" +
           parser.currentCommand.objects[i]
...
parser.scope =
  ...
  return list;
    ...
    parser.isInWorld = function(item) { return true; }
                       // Anywhere in the world
                       parser.isReachable =
        function(item) {
  return item.scopeStatus === REACHABLE && world.ifNotDark(item);
        ...
        parser.isVisible =
            function(item) { return item.scopeStatus && world.ifNotDark(item); }
            // Held or here, but not in a container
            parser.isPresent =
                function(
                    item) { return parser.isHere(item) || parser.isHeld(item); }
                // Used by examine, so the player can X ME, even if something
                // called metalhead is here.
                parser.isPresentOrMe =
                    function(item) {
  return parser.isHere(item) || parser.isHeld(item) || item === game.player;
}
                    // ... but not in a container
                    parser.isHeldNotWorn = function(item) {
  return item.isAtLoc(game.player.name, display.PARSER) &&
         world.ifNotDark(item) && !item.getWorn();
};
parser.isHeld =
    function(item) {
  return item.isAtLoc(game.player.name, display.PARSER) &&
         world.ifNotDark(item);
}
  ...
  for (let i = 0; i < npcs.length; i++) {
    if (item.isAtLoc(npcs[i].name, display.PARSER))
      return true;
  }
  return false;
}
...
parser.isNpcOrHereisNpcOrHere = function(item) {
  return (item.isAtLoc(game.player.loc, display.PARSER) &&
          world.ifNotDark(item)) ||
         item.npc;
};
parser.isNpcAndHere = function(
    item) { return item.isAtLoc(game.player.loc, display.PARSER) && item.npc; };
parser.isHere = function(item) {
  return item.isAtLoc(game.player.loc, display.PARSER) && world.ifNotDark(item);
};
// parser.isWorn =
...
// function(item, options) {
//  return
//  item.isAtLoc(options.container.name,
//  display.PARSER) &&
//  world.ifNotDark(item);
//}
...
parser.isContained = function(item) {
  const containers =
      parser.scope(parser.isReachable).filter(el => el.container);
  for (let i = 0; i < containers.length; i++) {
    if (containers[i].closed)
      continue;
    if (item.isAtLoc(containers[i].name, display.PARSER))
      return true;
  }
  return false;
};
parser.isHereOrContained =
    function(item) {
  if (parser.isHere(item))
    return true;
  if (parser.isContained(item))
    return true;
  return false;
}

    parser.isLiquid = function(item, options) {
  return item.isAtLoc(options.npc.name, display.PARSER) && item.liquid;
}
