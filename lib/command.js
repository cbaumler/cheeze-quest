...
function Cmd(name, hash) {
  ...
  this.script = function(objects, matches) {
    ...
    if (success) {
      return (this.noTurnscripts || suppressEndturn ? SUCCESS_NO_TURNSCRIPTS
                                                    : SUCCESS);
    } else {
      return FAILED;
    }
  ...
  this.processCommand = function(char, item, multi, match) {
    for (let i = 0; i < this.rules.length; i++) {
      if (typeof this.rules[i] !== "function") {
        errormsg("Failed to process command '" + this.name +
                 "' as one of its rules (" + i + ") is not a function.");
      }
    ...
    let result =
        printOrRun(char, item, this.attName, {multi : multi, match : match});
...
function NpcCmd(name, hash) {
  ...
  this.script = function(objects) {
    ...
    if (objects.length !== 2) {
      errormsg(
          "The command " + name +
          " is trying to use a facility for NPCs to do it, but there is no object list; this facility is only for commands in the form verb-object.");
      return FAILED;
    }
    const multi = (objects[1].length > 1 || parser.currentCommand.all);
    for (let i = 0; i < objects[1].length; i++) {
      if (npc["getAgreement" + this.cmdCategory] &&
          !npc["getAgreement" + this.cmdCategory](objects[1][i], this.name)) {
        // The getAgreement should give the response
        continue;
      }
      if (!npc["getAgreement" + this.cmdCategory] && npc.getAgreement &&
          !npc.getAgreement(this.cmdCategory, objects[1][i])) {
        continue;
      }
...
function NpcExitCmd(name, hash, dir) {
  ...
  this.script = function(objects) {
    ...
    if (!npc.getAgreementGo && npc.getAgreement &&
...
function useWithDoor(char, dir) {
  ...
  if (obj === undefined) {
    errormsg(
        "Not found an object called '" + this.door +
        "'. Any exit that uses the 'useWithDoor' function must also set a 'door' attribute.");
  }
...
function initCommands(EXITS) {
  ...
  EXITS.forEach(function(el) {
    if (!el.nocmd) {
      let regex =
          "^(" + GO_PRE_REGEX + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      if (el.alt) {
        regex += "|" + el.alt;
      }
      regex += ")$";
      ...
      regex = "^tell (.+) to (" + GO_PRE_REGEX + ")(" + el.name + "|" +
              el.abbrev.toLowerCase();
      if (el.alt) {
        regex += "|" + el.alt;
      }
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  if (item.isAtLoc(char.loc, display.PARSER))
    return true;
  if (item.isAtLoc(char.name, display.PARSER))
    return true;
