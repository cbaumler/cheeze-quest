...
const TAKEABLE_DICTIONARY = {
  getVerbs : function() {
    const verbs = this.use === undefined ? [ VERBS.examine ]
                                         : [ VERBS.examine, VERBS.use ];
...
const createEnsemble =
  ...
  res.getWorn =
      function(situation) {
    return this.isAtLoc(this.members[0].loc, situation) &&
           this.members[0].getWorn();
  }
    ...
    if (options && options.possessive)
      s += "'s";
    if (this.members[0].getWorn() && options.modified && this.isAllTogether()) {
      s += " (worn)";
    }
    // console.log(s)
    return s;
  ...
  // Tests if all parts are n the same location and either all are worn or none
  // are worn We can use this to determine if the set is together or not too
  res.isAtLoc =
      function(loc, situation) {
    if (situation !== display.PARSER)
      return false;
    const worn = this.members[0].getWorn();
    for (let i = 0; i < this.members.length; i++) {
      if (this.members[i].loc !== loc)
        return false;
      if (this.members[i].getWorn() !== worn)
        return false;
    }
    return true;
  }
    ...
    for (let i = 0; i < this.members.length; i++) {
      if (this.members[i].loc !== loc)
        return false;
      if (this.members[i].breakEnsemble && this.members[i].breakEnsemble())
        return false;
      if (this.members[i].getWorn() !== worn)
        return false;
    }
    ...
    for (let i = 0; i < this.members.length; i++) {
      ...
      if (this.members[i].scenery)
        delete this.members[i].scenery;
    }
    return true;
  }
...
// countableLocs should be a dictionary, with the room name as the key, and the
// number there as the value
const COUNTABLE = function(countableLocs) {
  ...
  res.getListAlias = function(loc) {
    return sentenceCase(this.pluralAlias ? this.pluralAlias
                                         : this.listalias + "s") +
           " (" + this.countAtLoc(loc) + ")";
  };
  ...
  res.isAtLoc = function(loc, situation) {
    ...
    if (situation === display.LOOK && this.scenery)
      return false;
    if (situation === display.SIDE_PANE && this.scenery)
      return false;
  ...
  res.findSource =
      function(sourceLoc, tryContainers) {
    ...
    if (tryContainers) {
      ...
      for (let i = 0; i < containers.length; i++) {
        if (containers[i].closed)
          continue;
        if (this.isAtLoc(containers[i].name))
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.wear = function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + this.wearMsg(char, this),
        {garment : this, actor : char});
    this.worn = true;
    if (this.afterWear)
      this.afterWear(char);
    return true;
  ...
  res.remove = function(isMultiple, char) {
    ...
    msg(prefix(this, isMultiple) + this.removeMsg(char, this),
        {garment : this, actor : char});
    this.worn = false;
    if (this.afterRemove)
      this.afterRemove(char);
    return true;
  };
...
const VESSEL =
  ...
  res.byname = function(options) {
    ...
    if (options && options.possessive)
      s += "'s";
    if (options.modified && res.volumeContained) {
      s += " (" + this.volumeContained + " " + VOLUME_UNITS + " " +
           res.containedLiquidName + ")";
    }
    return s;
  };
...
const CONTAINER = function(openable) {
  ...
  res.openMsg = function(
      isMultiple,
      char) { msg(prefix(this, isMultiple) + OPEN_SUCCESSFUL(char, this)); };
  ...
  res.closeMsg = function(
      isMultiple,
      char) { msg(prefix(this, isMultiple) + CLOSE_SUCCESSFUL(char, this)); };

  res.icon = function() {
    return ('<img src="images/' + (this.closed ? 'closed' : 'opened') +
            '12.png" />');
  };
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.byname = function(options) {
    ...
    if (options && options.possessive)
      s += "'s";
    if (!this.closed && options.modified) {
      s += " (open)";
    }
    return s;
  };
  ...
  const res = {
    ...
    testKeys : function(char, toLock) {
      for (let i = 0; i < keyNames.length; i++) {
        if (!w[keyNames[i]]) {
          errormsg("The key name for this container, `" + keyNames[i] +
                   "`, does not match any key in the game.");
          return false;
        }
...
const FURNITURE =
  ...
  res.assumePosture = function(isMultiple, char, posture, success_msg, adverb) {
    ...
    if (typeof this["on" + posture] === "function")
      this["on" + posture](char);
    return true;
  };
  if (options.sit) {
  ...
  if (options.stand) {
    res.standon = function(isMultiple, char) {
      return this.assumePosture(isMultiple, char, "standing",
                                STAND_ON_SUCCESSFUL);
    };
  }
  if (options.recline) {
    res.reclineon = function(isMultiple, char) {
      return this.assumePosture(isMultiple, char, "reclining",
                                RECLINE_ON_SUCCESSFUL);
    };
  }
...
const EDIBLE = function(isLiquid) {
  ...
  res.getVerbs = function() {
    ...
    if (this.isAtLoc(game.player.name)) {
      return [
        VERBS.examine, VERBS.drop, this.isLiquid ? VERBS.drink : VERBS.eat
      ];
    } else {
      return [ VERBS.examine, VERBS.take ];
    }
  ...
  const res = {
    ...
    push : function() {
      ...
      if (this.locked) {
        ...
        if (typeof w[this.loc].transitOnMove === "function")
          w[this.loc].transitOnMove(this.transitDest, exit.name);
...
function transitOfferMenu() {
  if (typeof this.transitCheck === "function" && !this.transitCheck()) {
    if (this.transitAutoMove)
      world.setRoom(game.player, game.player.previousLoc, this.transitDoorDir)
      return false;
  }
  const buttons = this.getTransitButtons(true, false);
  const transitDoorDir = this.transitDoorDir;
  const room = this;
  showMenu(this.transitMenuPrompt, buttons.map(el => el.transitDestAlias),
           function(result) {
             for (let i = 0; i < buttons.length; i++) {
               if (buttons[i].transitDestAlias === result) {
                 if (room[transitDoorDir].name === buttons[i].transitDest) {
                   ...
                   if (typeof room.transitOnMove === "function")
                     room.transitOnMove(buttons[i].transitDest,
                                        room[transitDoorDir].name);
                 }

                 room[transitDoorDir].name = buttons[i].transitDest
                 if (room.transitAutoMove) world.setRoom(
                     game.player, buttons[i].transitDest, room[transitDoorDir])
               }
             }
           });
}
...
const PLAYER = function() {
  const res = {
    ...
    player : true,
    // The following are used also by NPCs, so we can use the same functions for
    // both
    ...
    getHolding : function() {
      return this.getContents(display.LOOK)
          .filter(function(el) { return !el.getWorn(); });
    },

    getWearing : function() {
      return this.getContents(display.LOOK)
          .filter(function(el) { return el.getWorn() && !el.ensemble; });
    },

    getStatusDesc : function() {
      if (!this.posture)
        return false;
      return this.posture + " " + this.postureAdverb + " " +
             w[this.postureFurniture].byname({article : DEFINITE});
    },

    isAtLoc : function(loc, situation) {
      if (situation === display.LOOK)
        return false;
      if (situation === display.SIDE_PANE)
        return false;
      return (this.loc === loc);
    },
    ...
    getOuterWearable : function(slot) {
      ...
      console.log(scope(isWornBy, {npc:this}).filter(el =>
      el.getSlots().includes(slot)));
      */
    ...
    msg : function(s, params) { msg(s, params); },
    byname : function(options) {
      // console.log("npc byname " + this.name)
      if (options === undefined)
        options = {};

      if (this.pronouns === PRONOUNS.firstperson ||
          this.pronouns === PRONOUNS.secondperson) {
        return options && options.possessive ? this.pronouns.poss_adj
      ...
      if (options.modified) {
        ...
        if (held.length > 0) {
          list.push("holding " + formatList(held, {
                      article : INDEFINITE,
                      lastJoiner : LIST_AND,
                      modified : false,
        ...
        if (worn.length > 0) {
          list.push("wearing " + formatList(worn, {
                      article : INDEFINITE,
                      lastJoiner : LIST_AND,
                      modified : false,
                      nothing : LIST_NOTHING,
        ...
        if (list.length > 0)
          s += " (" +
               formatList(list, {lastJoiner : ";" + LIST_AND, sep : ";"}) + ")";
      }
