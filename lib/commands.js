...
// attName         If there is no script, then this attribute on the object will
// be used nothingForAll   If the player uses ALL and there is nothing there,
// use this error message noTurnscripts   Set to true to prevent turnscripts
// firing even when this command is successful

"use strict";

const cmdDirections = [] for (let i = 0; i < EXITS.length; i++) {
  if (EXITS[i].nocmd)
    continue;
  cmdDirections.push(EXITS[i].name);
  cmdDirections.push(EXITS[i].abbrev.toLowerCase());
  if (EXITS[i].alt)
    cmdDirections.push(EXITS[i].alt);
}
...
const commands = [
  ...
  new Cmd('Transcript', {
    ...
    script : function() {
      metamsg(
          "The TRANSCRIPT or SCRIPT command can be used to handle saving the input and output.");
      metamsg(
          "Use SCRIPT ON to turn on recording and SCRIPT OFF to turn it off. Use SCRIPT SHOW to display it. To empty the file, use SCRIPT CLEAR.");
      metamsg(
      ...
      metamsg(
          "Everything gets saved to memory, and will be lost if you go to another web page or close your browser, but should be saved when you save your game. You can only have one transcript dialog window open at a time.");
      return SUCCESS_NO_TURNSCRIPTS;
    },
  }),
  ...
  new Cmd('Inv', {
    ...
    script : function() {
      ...
      msg("You are carrying " + formatList(listOfOjects, {
            article : INDEFINITE,
            lastJoiner : LIST_AND,
            modified : true,
            nothing : LIST_NOTHING,
            loc : game.player.name
          }) +
          ".");
      return LOOK_COUNTS_AS_TURN ? SUCCESS : SUCCESS_NO_TURNSCRIPTS;
    },
  ...
  new Cmd('Examine', {
    ...
    objects : [ {ignore : true}, {scope : parser.isPresent} ],
  ...
  new Cmd('Look at', {
    ...
    regex : /^(look at|look) (.+)$/,
    attName : 'examine',
    objects : [ {ignore : true}, {scope : parser.isPresentOrMe} ],
    defmsg : DEFAULT_EXAMINE,
  }),
  new Cmd('Look out', {
  ...
  new Cmd('Wear2', {
    ...
    defmsg : function(char, item) {
      return item.ensemble ? CANNOT_WEAR_ENSEMBLE(char, item)
                           : CANNOT_WEAR(char, item);
    },
  }),
  new Cmd('Wear', {
    npcCmd : true,
  ...
  new Cmd('Talk to', {
    ...
    default : function(item) {
      msg("You chat to " + item.byname({article : DEFINITE}) +
          " for a few moments, before releasing that " +
          pronounVerb(item, "'be") + " not about to reply.");
      return false;
    },
  }),
  ...
  new Cmd('Topics', {
    ...
    default : function(item) {
      metamsg(
          "That's not something that will reply if you ask about anything.");
      return false;
    },
  }),
  ...
  new Cmd('FillWith', {
    ...
    script : function(
        objects) { return handleFillWithLiquid(game.player, objects); },
  }),
  ...
  new Cmd('PutIn', {
    ...
    script : function(
        objects) { return handlePutInContainer(game.player, objects); },
  }),
  ...
  new Cmd('TakeOut', {
    ...
    script : function(
        objects) { return handleTakeFromContainer(game.player, objects); },
  }),
  ...
  new Cmd('GiveTo', {
    ...
    script : function(
        objects) { return handleGiveToNpc(game.player, objects); },
  }),
  new Cmd('NpcGiveTo1', {
    regex : /^(.+), ?(give) (.+) (to) (.+)$/,
    rules : [ cmdRules.canManipulate, cmdRules.isHeld ],
  ...
  new Cmd('PushExit', {
    regex : new RegExp("^(push|pull|move|shift) (.+) (" +
                       cmdDirections.join("|") + ")$"),
    rules : [ cmdRules.canManipulate, cmdRules.isHereNotHeld ],
    cmdCategory : "Push",
    script : function(objects) { return handlePushExit(game.player, objects); },
    objects : [
      {text : true},
  ...
  new Cmd('NpcPushExit2', {
    regex : new RegExp("^tell (.+) to (push|pull|move|shift) (.+) (" +
                       cmdDirections.join("|") + ")$"),
    rules : [ cmdRules.canManipulate, cmdRules.isHereNotHeld ],
    cmdCategory : "Push",
    script : function(objects) {
  ...
  new Cmd('AskAbout', {
    ...
    script : function(arr) {
      ...
      if (!arr[0][0].askabout) {
        failedmsg("You can ask " + this.pronouns.objective + " about " +
                  arr[1] + " all you like, but " + pronounVerb(this, "'be") +
                  " not about to reply.");
        return false;
      }
  ...
  new Cmd('TellAbout', {
    ...
    script : function(arr) {
      ...
      if (!arr[0][0].askabout) {
        failedmsg("You can tell " + this.pronouns.objective + " about " +
                  arr[1] + " all you like, but " + pronounVerb(this, "'be") +
...
if (DEBUG) {
  commands.unshift(new Cmd('Walk Through', {
    ...
    script : function(objects) {
      const wt = walkthroughs[objects[0]] if (wt === undefined) {
  ...
  commands.unshift(new Cmd('Inspect', {
    ...
    script : function(arr) {
      const item = arr[0][0];
      debugmsg("See the console for details on the object " + item.name +
               " (press F12 to display the console)");
      console.log(item);
      return SUCCESS_NO_TURNSCRIPTS;
    },
    objects : [
  ...
  commands.unshift(new Cmd('InspectCmd', {
    ...
    script : function(arr) {
      ...
      for (let i = 0; i < commands.length; i++) {
        if (commands[i].name.toLowerCase() === arr[0] ||
            (commands[i].cmdCategory &&
             commands[i].cmdCategory.toLowerCase() === arr[0])) {
...
function handleFillWithLiquid(char, objects) {
  ...
  for (let i = 0; i < objects[0].length; i++) {
    ...
    if (flag) {
      if (!objects[0][i].isAtLoc(char.name)) {
        failedmsg(prefix(objects[0][i], multiple) +
                  NOT_CARRYING(char, objects[1][i]));
      } else {
  ...
  if (success && container.putInResponse)
    container.putInResponse();
  return success ? SUCCESS : FAILED;
}
  ...
  for (let i = 0; i < objects[0].length; i++) {
    ...
    if (flag) {
      if (!objects[0][i].isAtLoc(container.name)) {
        failedmsg(prefix(objects[0][i], multiple) +
                  NOT_INSIDE(container, objects[1][i]));
      } else {
...
function handleStandUpNpc(objects) {
  ...
  if (npc.getAgreementPosture && !npc.getAgreementPosture("stand")) {
    // The getAgreement should give the response
    return FAILED;
  } else if (!npc.getAgreementPosture && npc.getAgreement &&
             !npc.getAgreement("Posture", "stand")) {
    return FAILED;
  }
  if (!npc.canPosture()) {
    return FAILED;
