...
const DEFAULT_OBJECT = {
  ...
  isAtLoc : function(loc, situation) {
    if (typeof loc !== "string")
      loc = loc.name;
    if (!w[loc])
      errormsg("The location name `" + loc +
               "`, does not match anything in the game.");
    if (this.loc !== loc)
      return false;
  ...
  isHere : function() { return this.isAtLoc(game.player.loc); },
  ...
  scopeSnapshot : function(visible) {
    ...
    let l
    if (this.getContents) {
      if (!this.canSeeThrough() && !this.scopeStatusForRoom) {
        return;
      }
      if (!this.canReachThrough() && this.scopeStatusForRoom !== REACHABLE) {
        visible = true;
      }
  ...
  moveToFrom : function(toLoc, fromLoc) {
    ...
    if (!w[fromLoc])
      errormsg("The location name `" + fromLoc +
               "`, does not match anything in the game.");
    if (!w[toLoc])
      errormsg("The location name `" + toLoc +
               "`, does not match anything in the game.");
    this.loc = toLoc;
  ...
  templatePostLoad : function() { this.postLoad(); },

  preSave : NULL_FUNC,

  templatePreSave : function() { this.preSave(); },
  ...
  templatePreSave : function() {
    for (let i = 0; i < EXITS.length; i++) {
      ...
      if (this[dir] !== undefined) {
        ...
        this["customSaveExit" + dir] +=
            "/" + (this[dir].hidden ? "hidden" : "");
        if (this.saveExitDests)
          this["customSaveExitDest" + dir] = this[dir].name;
      }
    }
    this.preSave();
  },
    ...
    for (let i = 0; i < EXITS.length; i++) {
      ...
      if (this["customSaveExit" + dir]) {
        ...
        if (this.saveExitDests) {
          ...
          // console.log("Just set " + dir + " in " + this.name + " to " +
          // this["customSaveExitDest" + dir])
...
const DEFAULT_ITEM = {
  ...
  getVerbs : function() {
    return this.use === undefined ? [ VERBS.examine ]
